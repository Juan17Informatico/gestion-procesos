      ...
      <div className="rounded-lg border border-rose-100 bg-gradient-to-br from-rose-50 via-sky-50 to-emerald-50 p-4 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid gap-2 sm:grid-cols-[1fr_auto_auto]">
            <TextInput placeholder="Buscar nombre, CC o telefono" value={query} onChange={(event) => setQuery(event.target.value)} />
            <Button type="button" variant="primary" onClick={() => setShowPaste(true)}>Pegar datos</Button>
            <Button type="button" onClick={() => { setDateFilter(''); setQuery(''); setStatusFilter('all'); }}>Ver todo</Button>
          </div>
        </div>
      </div>

      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Total" value={stats.total} tone="bg-white" />
        <StatCard label="Completos" value={stats.complete} tone="bg-emerald-50" />
        <StatCard label="Convalidaciones" value={stats.validation} tone="bg-sky-50" />
        <StatCard label="Pendientes" value={stats.pending} tone="bg-amber-50" />
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
        ...
        <div className="grid min-h-0 min-w-0 content-start gap-4">
          ...
          <div className="min-h-[420px] overflow-y-auto rounded-xl border border-slate-200 bg-slate-50/70 p-3 lg:max-h-[calc(100vh-360px)]">
            {grouped.length === 0 ? (
              <EmptyProcesses query={query} onClear={() => { setQuery(''); setStatusFilter('all'); setDateFilter(''); }} />
            ) : null}
              ...
              {grouped.map(([date, items]) => (
                <section className="grid gap-3" key={date}>
                  <div className="flex flex-wrap items-end justify-between gap-2">
                    <div>
                      <h3 className="text-lg font-semibold capitalize">{formatProcessDate(date)}</h3>
                  ...
                  <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
                    {items.map((process) => {
                      ...
                      return (
                        <article className="grid gap-3 border-b border-slate-100 p-3 last:border-b-0 md:grid-cols-[minmax(0,1fr)_150px_170px_150px] md:items-center" key={process.id}>
                          ...
                          <p className="text-sm text-slate-700">CC: {process.identification || 'Falta'}</p>
                          <div className="flex items-center gap-2 text-sm text-slate-700">
                            <a className="font-medium hover:underline" href={process.phone ? `tel:${process.phone}` : undefined}>{process.phone || 'Sin telefono'}</a>
                            {process.phone ? <button className="rounded-md bg-slate-100 px-2 py-1 text-xs hover:bg-slate-200" type="button" onClick={() => copyPhone(process.phone ?? '')}>Copiar</button> : null}
                          </div>
          ...
          <Pagination
            ...
            hasActiveSearch={Boolean(query.trim() || dateFilter || statusFilter !== 'all')}
            itemLabel="procesos"
              ...
              <div className="mt-5 grid gap-3">
                ...
                <div className="max-h-72 overflow-auto rounded-lg border border-slate-200">
                  {preview.map((item) => {
                    ...
                    return (
                      <div className="grid gap-2 border-b border-slate-100 p-3 last:border-b-0 md:grid-cols-[1fr_120px_130px_160px]" key={item.process.id}>
                        ...
                        {(item.warnings.length > 0 || item.duplicateOf) ? <p className="text-sm text-amber-800 md:col-span-4">Atencion: {[...item.warnings, item.duplicateOf ? 'posible duplicado' : ''].filter(Boolean).join(', ')}</p> : null}
                      </div>
                    );
                  })}
                </div>
...
function EmptyProcesses({ query, onClear }: { query: string; onClear: () => void }) {
  return (
