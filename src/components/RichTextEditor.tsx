...
export function RichTextEditor({ value, onChange, placeholder = 'Escribe una nota...' }: RichTextEditorProps) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          'rich-text-content min-h-48 rounded-b-2xl bg-[var(--color-surface)] px-4 py-3 text-sm text-[var(--color-text)] outline-none',
  ...
  return (
    <div className="relative rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-sm">
      <div className="relative flex flex-wrap gap-1.5 border-b border-[var(--color-border)] bg-[var(--color-surface-soft)] p-2.5" ref={toolbarRef}>
        ...
        <div className="relative">
          <button
            className={`min-h-9 rounded-md border px-3 text-sm font-medium transition ${
              editor?.isActive('highlight')
                ...
                : 'border-[var(--color-border)] bg-[var(--color-surface)] text-[var(--color-muted)] hover:bg-[var(--color-primary-soft)]'
            }`}
            disabled={!editor}
          ...
          {isHighlightOpen ? (
            <div className="absolute left-0 top-11 z-30 w-64 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-3 shadow-xl">
              <p className="mb-2 text-xs font-semibold text-[var(--color-muted)]">Color de resaltado</p>
              <div className="grid grid-cols-6 gap-2">
                {highlightColors.map((color) => (
                  <button
                    aria-label={`Resaltar en ${color.label}`}
                    className="h-8 w-8 rounded-full border border-[var(--color-border)] shadow-sm transition hover:scale-105"
                    disabled={!editor}
                    key={color.value}
              ...
              <button
                className="mt-3 w-full rounded-md border border-[var(--color-border)] px-3 py-2 text-xs font-medium text-[var(--color-muted-strong)] hover:bg-[var(--color-surface-muted)]"
                disabled={!editor}
                onClick={() => {
        ...
        <button
          className="min-h-9 rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] px-3 text-sm font-medium text-[var(--color-muted)] transition hover:bg-[var(--color-warning-bg)] hover:text-[var(--color-text)]"
          disabled={!editor}
      ...
      {editor?.isEmpty ? (
        <p className="pointer-events-none absolute ml-3 mt-3 text-sm text-[var(--color-subtle)]">{placeholder}</p>
      ) : null}
